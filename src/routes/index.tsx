...
function Landing() {
  ...
  const google = async () => {
    ...
    if (res.error) setErr((res.error as any).message ?? "Não foi possível entrar com o Google.");
        ...
        <div className="mt-8 grid flex-1 items-center gap-10 lg:grid-cols-2">
          <div className="max-w-xl">
            ...
            <h1 className="mb-4 text-4xl font-black leading-[1.05] tracking-tight sm:text-6xl md:text-7xl" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
              Todo dorama.<br />
              <span className="bg-gradient-to-r from-primary to-white bg-clip-text text-transparent">Uma só tela premium.</span>
            </h1>
            <p className="mb-6 max-w-md text-base text-white/70 sm:text-lg">
              Assista aos melhores K-Dramas, J-Dramas e C-Dramas em uma experiência cuidadosamente premium.
            </p>
            <div className="hidden gap-3 sm:flex">
              <div className="rounded-full glass px-3 py-1.5 text-xs">1080p / 4K</div>
              <div className="rounded-full glass px-3 py-1.5 text-xs">Continue em qualquer aparelho</div>
              <div className="rounded-full glass px-3 py-1.5 text-xs">Sem anúncios</div>
            </div>
          </div>
          ...
          <div className="mx-auto w-full max-w-md rounded-2xl glass-strong p-6 shadow-[0_30px_80px_-20px_rgba(0,0,0,0.8)]">
            ...
            <form onSubmit={submit} className="space-y-3">
              {mode === "signup" && (
                <input type="text" placeholder="Nome de exibição" value={displayName} onChange={e => setDisplayName(e.target.value)}
              ...
              <div className="relative">
                <input type={show ? "text" : "password"} required minLength={6} placeholder="Senha" value={password} onChange={e => setPassword(e.target.value)}
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? "Aguarde…" : mode === "signin" ? "Entrar na Doramaflix" : "Criar conta"}
              </button>
            </form>
            ...
            <p className="mt-4 text-center text-xs text-white/40">Ao continuar, você concorda com os termos de uso e a política de privacidade.</p>
          </div>
        </div>

        <footer className="mt-10 text-center text-xs text-white/40">© {new Date().getFullYear()} Doramaflix. Todo o conteúdo é licenciado e pertence aos seus respectivos detentores.</footer>
      </div>
    </div>
  );
}
